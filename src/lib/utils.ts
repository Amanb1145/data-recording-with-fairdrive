...
export const addTodo = async (todo: Todo, todos: Todo[]) => {
  ...
  const file = document.getElementById("todo").files[0];
...
export const deleteTodo = async (deleteTodo: String, todos: Todo[]) => {
  let data = {
    podName: config.todoAppNamespace,
    filePath: `${config.todoItemsDirectory}/${deleteTodo}`,
  };
  ...
  if (response.ok) {
    console.log({ response, json });
    return todos.filter((todo) => todo.name != deleteTodo);
  } else {
    console.error({ response, json });
  }
};
...
export const readTodo = async (todofile: string) => {
  ...
  console.log(todofile, data, "llalallala")
  let FAIROS_HOST = apiHost();
  ...
  // if (response.ok) {
  //   console.log({ response, json }, "0002222233333");
  //   return json;
  // } else {
  //   console.error({ response, json });
  // }
